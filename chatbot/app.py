...
def related_sym(psym1):
    ...
    if num != 0:
        s += "Select the option which you match with:."
        return s
    else:
        return 0
...
def get_bot_response():
    ...
    if "step" in session:
        if session["step"] == "Q_C":
            ...
            session.clear()
            if s == "q":
                "Thank you for using MediCo - MedBot " + name
            else:
                session["step"] = "FS"
                session["name"] = name
                session["age"] = age
    ...
    if session['step'] == "Depart":
        session['step'] = "BFS"
        return "Well, Hello " + session[
            "name"] + ", now I will be asking you some questions about your symptoms to see what you should do. Type S to start diagnosis!"
    if session['step'] == "BFS":
        session['step'] = "FS"  # first symp
        return "Can you specify your main symptom " + session["name"] + " ?"
    if session['step'] == "FS":
        sym1 = s
        ...
        if sim1 == 1:
            ...
            return "If you are facing any other symptoms, can you specify it?"
    if session['step'] == "RS1":
        temp = session['FSY']
        psym1 = temp[2]
        psym1 = psym1[int(s)]
    ...
    if session['step'] == "END":
        if result is not None:
            if result[0] != session["testpred"]:
                session['step'] = "Q_C"
                return "As you provided me with few symptoms, I am sorry to announce that I am unable predict your " \
                       "disease at the moment!!! <br> Can you specify more about what you are feeling or type Q to " \
                       "stop the conversation "
            session['step'] = "Description"
            session["disease"] = result[0]
            return "Well " + session["name"] + ", you may have " + result[
                0] + ". Type D to get a description of the disease."
        else:
    ...
    if session['step'] == "Description":
        ...
        if session["disease"] in description_list.keys():
            return description_list[session["disease"]] + " \n <br>  How many days have you had these symptoms?"
        else:
    ...
    if session['step'] == "Severity":
        ...
        if calc_condition(session["all"], int(s)) == 1:
            return "You should take consultation from a doctor <br> Type Q to exit"
        else:
            ...
            for e in precautionDictionary[session["disease"]]:
                msg += '\n ' + str(i) + ': ' + e + '<br>'
                i += 1
            msg += ' Type Q to end diagnosis.'
            return msg
    if session['step'] == "FINAL":
        session['step'] = "BYE"
        return "Your diagnosis has been completed. Do you need another medical consultation (yes or no)? "
    if session['step'] == "BYE":
        name = session["name"]
        age = session["age"]
        gender = session["gender"]
        ...
        if s.lower() == "yes":
            ...
            session["age"] = age
            session['step'] = "FS"
            return "Hello " + session["name"] + " Please tell me your main symptom. "
        else:
            return "Thanks " + name + " for using MediCo - MedBot.</b>"
